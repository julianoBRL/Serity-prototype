...
import io.github.julianobrl.seritycommon.services.TicketService;
...
public class TicketController {
	...
	@GetMapping
	private ResponseEntity<?> getAll(@RequestBody(required = false) Ticket params) {
		Specification<Ticket> spec = new GSpecification<Ticket>().setSpecs(params);
	...
	@PostMapping
	private ResponseEntity<?> register(@Valid @RequestBody Ticket model){
	...
	@PutMapping("/{id}")
	private ResponseEntity<?> edit(@PathVariable("id") Long ID, @RequestBody Ticket model){
		try {
