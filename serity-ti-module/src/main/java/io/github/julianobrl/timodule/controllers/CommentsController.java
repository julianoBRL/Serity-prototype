package io.github.julianobrl.timodule.controllers;
...
import io.github.julianobrl.seritycommon.services.CommentService;
...
public class CommentsController {
	...
	@GetMapping
	private ResponseEntity<?> getAll(@RequestBody(required = false) Comment params) {
		Specification<Comment> spec = new GSpecification<Comment>().setSpecs(params);
	...
	@PostMapping
	private ResponseEntity<?> register(@Valid @RequestBody Comment model){
	...
	@PutMapping("/{id}")
	private ResponseEntity<?> edit(@PathVariable("id") Long ID, @RequestBody Comment model){
		try {
