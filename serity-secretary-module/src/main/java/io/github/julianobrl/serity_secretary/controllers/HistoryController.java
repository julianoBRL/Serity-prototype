...
import io.github.julianobrl.seritycommon.services.HistoryService;
...
public class HistoryController {
	...
	@GetMapping
	private ResponseEntity<?> getAll(@RequestBody(required = false) History params) {
		Specification<History> spec = new GSpecification<History>().setSpecs(params);
	...
	@PostMapping
	private ResponseEntity<?> register(@Valid @RequestBody History model){
	...
	@PutMapping("/{id}")
	private ResponseEntity<?> edit(@PathVariable("id") Long ID, @RequestBody History model){
		try {
