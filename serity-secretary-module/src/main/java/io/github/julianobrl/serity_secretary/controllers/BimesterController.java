...
import io.github.julianobrl.seritycommon.services.BimesterService;
import io.github.julianobrl.seritycommon.utils.GSpecification;

@RestController
@RequestMapping("/bimester")
public class BimesterController {
	
	@Autowired
	private BimesterService service;
	
	@GetMapping
	private ResponseEntity<?> getAll(@RequestBody(required = false) Bimester params) {
		Specification<Bimester> spec = new GSpecification<Bimester>().setSpecs(params);
	...
	@PostMapping
	private ResponseEntity<?> register(@Valid @RequestBody Bimester model){
	...
	@PutMapping("/{id}")
	private ResponseEntity<?> edit(@PathVariable("id") Long ID, @RequestBody Bimester model){
		try {
