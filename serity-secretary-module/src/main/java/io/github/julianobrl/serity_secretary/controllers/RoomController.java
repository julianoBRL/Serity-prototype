...
import io.github.julianobrl.seritycommon.services.RoomService;
...
public class RoomController {
	...
	@GetMapping
	private ResponseEntity<?> getAll(@RequestBody(required = false) Room params) {
		Specification<Room> spec = new GSpecification<Room>().setSpecs(params);
	...
	@PostMapping
	private ResponseEntity<?> register(@Valid @RequestBody Room model){
	...
	@PutMapping("/{id}")
	private ResponseEntity<?> edit(@PathVariable("id") Long ID, @RequestBody Room model){
		try {
