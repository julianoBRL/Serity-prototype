...
import io.github.julianobrl.seritycommon.services.SubjectService;
...
public class SubjectController {
	...
	@GetMapping
	private ResponseEntity<?> getAll(@RequestBody(required = false) Subject params) {
		Specification<Subject> spec = new GSpecification<Subject>().setSpecs(params);
	...
	@PostMapping
	private ResponseEntity<?> register(@Valid @RequestBody Subject model){
	...
	@PutMapping("/{id}")
	private ResponseEntity<?> edit(@PathVariable("id") Long ID, @RequestBody Subject model){
		try {
