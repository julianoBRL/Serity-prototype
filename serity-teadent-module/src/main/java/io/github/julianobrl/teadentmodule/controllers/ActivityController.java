...
import io.github.julianobrl.seritycommon.services.ActivityService;
...
public class ActivityController {
	...
	@GetMapping
	private ResponseEntity<?> getAll(@RequestBody(required = false) Activity params) {
		Specification<Activity> spec = new GSpecification<Activity>().setSpecs(params);
	...
	@PostMapping
	private ResponseEntity<?> register(@Valid @RequestBody Activity model){
	...
	@PutMapping("/{id}")
	private ResponseEntity<?> edit(@PathVariable("id") Long ID, @RequestBody Activity model){
		try {
